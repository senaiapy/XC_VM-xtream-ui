...
function loadCron() {
    ...
    if ($db->query("SELECT `signal_id`, `custom_data` FROM `signals` WHERE `server_id` = ? AND `custom_data` <> '' AND `cache` = 0 ORDER BY signal_id ASC;", SERVER_ID)) {
        ...
        if (!$rCheck['php']) {
            ...
            $phpBinaryName = basename(readlink(PHP_BIN));
            $phpVersionString = null;
            switch (CoreUtilities::$rServers[SERVER_ID]['php_version']) {
                case '74':
                    $phpVersionString = 'php_7.4';
                    break;
                case '72':
                    $phpVersionString = 'php_7.2';
                    break;
                case '80':
                    $phpVersionString = 'php_8.0';
                    break;
                default:
                    break;
            }
