...
if (isset($_SESSION['hash'])) {
	...
	if (isset(CoreUtilities::$rRequest['action'])) {
		...
		if (CoreUtilities::$rRequest['action'] == 'epglist') {
			if (hasPermissions('adv', 'import_streams')) {
				...
				if (!isset(CoreUtilities::$rRequest['search'])) {
					...
					$rLanguages = array('#' => 'Undefined Language');
					$db->query('SELECT `language`, `name` FROM `epg_languages`;');

					foreach ($db->get_rows() as $rRow) {
						$rLanguages[$rRow['language']] = $rRow['name'];
					}
					$rLangMap = array();
					$db->query('SELECT `callSign`, `bcastLangs`, `name`, `picon` FROM `epg_api` WHERE (`callSign` LIKE ? OR `name` LIKE ?) GROUP BY CONCAT(`name`, `bcastLangs`) ORDER BY `name` ASC LIMIT 50;', CoreUtilities::$rRequest['search'] . '%', CoreUtilities::$rRequest['search'] . '%');

					foreach ($db->get_rows() as $rRow) {
						$rLangs = json_decode($rRow['bcastLangs'], true);
						...
						foreach ($rLangs as $rLang) {
							if (isset($rLangMap[$rLang])) {
							} else {
								$rLangMap[$rLang] = array();
							}

							$rLangMap[$rLang][] = array('id' => $rRow['callSign'], 'text' => $rRow['name'], 'icon' => $rRow['picon'], 'lang' => $rLang, 'type' => 1);
						}
					}

					foreach ($rLangMap as $rLang => $rResults) {
						$rReturn['items'][] = array('text' => 'EPG API - ' . $rLanguages[$rLang], 'children' => $rResults);
						$rReturn['total_count'] += count($rResults);
					}
				}
		...
		if (CoreUtilities::$rRequest['action'] == 'epg_api_single') {
			$rID = 0;

			if (0 < strlen(CoreUtilities::$rRequest['id']) && !is_numeric(CoreUtilities::$rRequest['id'])) {
				$db->query('SELECT `stationId` FROM `epg_api` WHERE `callSign` = ?;', CoreUtilities::$rRequest['id']);

				if ($db->num_rows() > 0) {
					$rID = intval($db->get_row()['stationId']);
				}
			} else {
				$rID = intval(CoreUtilities::$rRequest['id']);
			}
			...
			if ($rID > 0) {
				$rPost = array('action' => 'epg', 'id' => $rID, 'days' => 2);
				// $rChannel = getEPGAPI($rPost);
				$rChannel = "";

				$rFirst = null;

				foreach ($rChannel[array_keys($rChannel)[0]] as $rResult) {
					if (!(time() < strtotime($rResult['endTime']) && (!$rFirst || strtotime($rResult['startTime']) < $rFirst + 86400))) {
					} else {
						if ($rFirst) {
						} else {
							$rFirst = strtotime($rResult['startTime']);
						}

						$rResult['startTime'] = date('H:i:s', strtotime($rResult['startTime']));

						if ($rResult['program']['shortDescription'] && strlen($rResult['program']['shortDescription']) != 0) {
						} else {
							$rResult['program']['shortDescription'] = 'No description.';
						}
			...
			echo json_encode(array('result' => true, 'data' => $rResults));

			exit();
		}
		if (CoreUtilities::$rRequest['action'] == 'report_epg') {
			$rData = array('action' => 'report', 'id' => intval(CoreUtilities::$rRequest['id']), 'type' => CoreUtilities::$rRequest['type'], 'report' => CoreUtilities::$rRequest['report']);
			$rOptions = array('http' => array('header' => 'Content-type: application/x-www-form-urlencoded' . "\r\n", 'method' => 'POST', 'content' => http_build_query($rData)));
			$rContext = stream_context_create($rOptions);
			$rResult = file_get_contents($rSettings['epg_api_url'], false, $rContext);
			echo json_encode(array('result' => true));

			exit();
		}
		if (CoreUtilities::$rRequest['action'] == 'guess_epg') {
			$rName = str_replace(array(':', '|', '-', ' hd', ' fhd', ' 4k', ' sd'), array('#', '#', '#', '', '', '', ''), strtolower(CoreUtilities::$rRequest['name']));
			$rTVGID = (!empty(CoreUtilities::$rRequest['id']) ? strtolower(CoreUtilities::$rRequest['id']) : null);

			if (stripos($rName, '#') !== false) {
				...
				$rSearch = $rNameArray[0];
			} else {
				$rSearch = $rName;
			}

			$rSearch = preg_replace('/[^A-Za-z0-9 ]/', '', $rSearch);
			$rLanguage = null;
			$rGuessLanguages = array('af' => 'af', 'sq' => 'sq', 'ar' => 'ar', 'arc' => 'arc', 'arp' => 'arp', 'hy' => 'hy', 'eu' => 'eu', 'be' => 'be', 'bn' => 'bn', 'bg' => 'bg', 'km' => 'km', 'zh' => 'zh', 'hr' => 'hr', 'cs' => 'cs', 'da' => 'da', 'nl' => 'nl', 'en' => 'en', 'fa' => 'fa', 'fi' => 'fi', 'fr' => 'fr-FR', 'gd' => 'gd', 'de' => 'de', 'el' => 'el', 'he' => 'he', 'hi' => 'hi', 'hu' => 'hu', 'iu' => 'iu', 'it' => 'it', 'ja' => 'ja', 'ko' => 'ko', 'ku' => 'ku', 'mk' => 'mk', 'ml' => 'ml', 'no' => 'no', 'pl' => 'pl', 'pt' => 'pt', 'br' => 'pt-BR', 'pa' => 'pa', 'ro' => 'ro', 'ru' => 'ru', 'sr' => 'sr', 'so' => 'so', 'es' => 'es', 'sv' => 'sv', 'tl' => 'tl', 'ta' => 'ta', 'te' => 'te', 'tr' => 'tr', 'ur' => 'ur', 'vi' => 'vi', 'yi' => 'yi', 'uk' => 'en-GB', 'us' => 'en', 'ca' => 'en');
			$rPieces = explode(' ', preg_replace('/[^A-Za-z0-9 ]/', '', CoreUtilities::$rRequest['name']));

			if (1 >= count($rPieces)) {
			} else {
				foreach (array(0, count($rPieces) - 1) as $i) {
					$rPiece = strtolower($rPieces[$i]);

					if (!isset($rGuessLanguages[$rPiece])) {
					} else {
						$rLanguage = $rGuessLanguages[$rPiece];
			...
			if (!$rLanguage) {
				...
				$db->query("SELECT `callSign`, `name`, `picon` FROM `epg_api` WHERE (LOWER(`name`) LIKE ? OR `callSign` = ?) AND `bcastLangs` LIKE '%\"" . $rLanguage . "\"%' ORDER BY `name` ASC;", trim($rSearch) . '%', $rTVGID);

				foreach ($db->get_rows() as $rEPGItem) {
					if ($rTVGID && strtolower($rEPGItem['callSign']) == $rTVGID) {
						$rEPG = array('id' => $rEPGItem['callSign'], 'text' => $rEPGItem['name'], 'icon' => $rEPGItem['picon'], 'lang' => $rLanguage, 'type' => 1, 'epg_id' => 0);
					} else {
						similar_text(strtolower($rEPGItem['name']), trim(strtolower($rSearch)), $rPercentage);

						if (!($rMaxPerc < $rPercentage && 90 <= $rPercentage)) {
						} else {
							$rEPG = array('id' => $rEPGItem['callSign'], 'text' => $rEPGItem['name'], 'icon' => $rEPGItem['picon'], 'lang' => $rLanguage, 'type' => 1, 'epg_id' => 0);
							$rMaxPerc = $rPercentage;
						}
					}
				}
			}
			...
			if ($rEPG) {
				...
				$db->query('SELECT `callSign`, `bcastLangs`, `name`, `picon` FROM `epg_api` WHERE LOWER(`name`) LIKE ? OR `callSign` = ? ORDER BY `name` ASC, `eng` DESC;', trim($rSearch) . '%', $rTVGID);

				foreach ($db->get_rows() as $rEPGItem) {
					$rLangs = json_decode($rEPGItem['bcastLangs'], true);

					if ($rTVGID && strtolower($rEPGItem['callSign']) == $rTVGID) {
						$rEPG = array('id' => $rEPGItem['callSign'], 'text' => $rEPGItem['name'], 'icon' => $rEPGItem['picon'], 'lang' => (isset($rLangs[0]) ? $rLangs[0] : ''), 'type' => 1, 'epg_id' => 0);
					} else {
						similar_text(strtolower($rEPGItem['name']), trim(strtolower($rSearch)), $rPercentage);
			...
			if ($rEPG) {
				...
				$db->query('SELECT `epg_channels`.`epg_id`, `epg_channels`.`channel_id`, `epg_channels`.`name`, `epg_channels`.`langs`, `epg`.`epg_name` FROM `epg_channels` LEFT JOIN `epg` ON `epg_channels`.`epg_id` = `epg`.`id` WHERE LOWER(`epg_channels`.`name`) LIKE ? OR LOWER(`epg_channels`.`channel_id`) = ? ORDER BY `epg_channels`.`name` ASC;', trim($rSearch) . '%', $rTVGID);

				foreach ($db->get_rows() as $rEPGItem) {
					if ($rTVGID && strtolower($rEPGItem['channel_id']) == $rTVGID) {
						$rLangs = json_decode($rEPGItem['langs'], true);
						$rEPG = array('id' => $rEPGItem['channel_id'], 'text' => $rEPGItem['name'], 'icon' => null, 'lang' => (isset($rLangs[0]) ? $rLangs[0] : ''), 'epg_id' => $rEPGItem['epg_id'], 'type' => 0);
			...
			if ($rEPG) {
				...
				$rEPGNames = getEPGNames(trim($rSearch));

				foreach ($rEPGNames as $rEPGItem) {
					similar_text(strtolower($rEPGItem['name']), trim(strtolower($rSearch)), $rPercentage);
			...
			if ($rEPG) {
				echo json_encode(array('result' => true, 'data' => $rEPG));
